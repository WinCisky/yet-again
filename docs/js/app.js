...
function showSubscribed() {
    notSubscribedDiv.classList.add("hidden");
    subscribedDiv.classList.remove("hidden");
}

function showNotSubscribed() {
    subscribedDiv.classList.add("hidden");
    notSubscribedDiv.classList.remove("hidden");
...
function renderTypes(types, endpoint) {
    ...
    for (const t of types) {
        ...
        row.className = "flex items-center gap-4 border p-2";
...
// Re-check when the page is restored from bfcache (back/forward navigation)
window.addEventListener("pageshow", (event) => {
    if (event.persisted) checkExistingSubscription();
});
